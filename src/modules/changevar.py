...
Introduced relative errors (for x values) in all methods;
...
import iapws
import utils.useful_tools as ut
...
# tolerance (can be deleted) = specified tolerance for x to reach zero solution.
...
def find_zero_bisection_rho(x_min, x_max, T, xmol, P_target, tolerance, limit_iterations):    # Defines this function 
...
def find_zero_newtonraphson_rho(x_guess, T_target, xmol_target, P_target, tolerance, limit_iterations):     # Define this functions with those parameters
    i = 1   # Define the iteration variable
    while i <= limit_iterations:    # Definition of loop according to number of iterations
        x_guess_minusStep = x_guess*(1-tolerance)   # Calculate a nearby x (lower value) to estimate the differential funciton numerically with "tolerance" variation in x. NOTE: other values than tolerance can be used.
        x_guess_plusStep = x_guess*(1+tolerance)   # Calculate a nearby x (lower value) to estimate the differential funciton numerically with "tolerance" variation in x. NOTE: other values than tolerance can be used.
        if x_guess_minusStep <= 0 or x_guess_plusStep <= 0:     # When these values are negative, FOR DENSITY, it has diverged.
        ...
        differential_function = (function_P_rho(x_guess_plusStep, T_target, xmol_target, P_target)-function_P_rho(x_guess_minusStep, T_target, xmol_target, P_target))/(x_guess_plusStep-x_guess_minusStep)     # According to Newton-Raphson Method the numerical differential was created.
        x_guess_new = x_guess-function_P_rho(x_guess, T_target, xmol_target, P_target)/differential_function    # According to Newton-Raphson Method, a new x_guess was created.
        if abs((x_guess_new-x_guess)/x_guess) < tolerance:    # Check if the specified number difference reached the relative tolerance.
            return x_guess_new  # Return x_guess value found
        else:   # Otherwise:
            x_guess = x_guess_new   # Update x_guess to be the new one found in last iteration loop.
...
# --------------------------------------------------------------------------
def my_prop_newraph(P_target, T_target, xmass_target, rho_guess_min, rho_guess_max, n_divisions, tolerance, limit_iterations):  # Defines the function
    MW_NH3 = 17.03026 # [g/mol]     Referenced number from iapws, inside nh3h2o documentation.
    MW_H2O = 18.015268 # [g/mol]    Referenced number from iapws, inside nh3h2o documentation.
    xmol_target = ut.xmass_to_xmol(xmass_target,MW_NH3,MW_H2O) # [mol/mol]  # Transformation of mass fraction into molar fraction
...
# Convert ranges into a number (average of range) and apply Newton-Raphson method just with these values.
    ...
    for x in x_guess:   # For each average zero number in x_guess, do:
        x_zero = find_zero_newtonraphson_rho(x, T_target, xmol_target, P_target, tolerance, limit_iterations)    # Find a zero using this number as a guess with Newton-Raphson method
...
# --------------------------------------------------------------------------
def my_prop_hybrid(P_target, T_target, xmass_target, rho_guess_min, rho_guess_max, n_divisions, tolerance_BS, tolerance_NR, limit_iterations):
    MW_NH3 = 17.03026 # [g/mol]     Referenced number from iapws, inside nh3h2o documentation.
    MW_H2O = 18.015268 # [g/mol]    Referenced number from iapws, inside nh3h2o documentation.
    xmol_target = ut.xmass_to_xmol(xmass_target,MW_NH3,MW_H2O) # [mol/mol]  # Transformation of mass fraction into molar fraction
    ...
    for x_min, x_max in x_ranges:   # For each x_min and x_max in the list of list from x_ranges, do:
        function_ranges.append([function_P_rho(x_min, T_target, xmol_target, P_target), function_P_rho(x_max, T_target, xmol_target, P_target)])    # Add a list of function solution pair in x_min and x_max (range)
    x_zero_ranges = x_with_zero_ranges(x_ranges, function_ranges, tolerance_NR)  # Check if there are zeros between the ranges
...
# Check if I have a range or zero points and apply bisection method just within ranges
    ...
    for x in x_zero_ranges:     # Take each range into x_zero_ranges
        if not(isinstance(x, list)):    # If the "range" is not a list (i.e. is a number, for example), then is added directly into the answer,m because it is necessarily a zero.
            ...
            x_zero_BS = find_zero_bisection_rho(x[0],x[1],T_target,xmol_target,P_target,tolerance_BS,limit_iterations)     # Apply the bissection method to find a zero
...
# Apply Newton-Raphson method just within the values found with bissection method.
    ...
    for x in x_with_zero_BS:   # For each average zero number in x_guess, do:
        x_zero_NR = find_zero_newtonraphson_rho(x, T_target, xmol_target, P_target, tolerance_NR, limit_iterations)    # Find a zero using this number as a guess with Newton-Raphson method
